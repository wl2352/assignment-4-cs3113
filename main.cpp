...
void update()
{
    ...
    if (g_current_scene == g_level_a && g_current_scene->m_state.player->get_position().y < -10.0f) switch_to_scene(g_level_c);
    if (g_current_scene == g_level_c && g_current_scene->m_state.player->get_position().y < -10.0f) switch_to_scene(g_level_b);
}
